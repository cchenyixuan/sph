...
void main() {
    ...
    if(v_index<300000){
        ...
        color = vec4(1.0, 1.0, 0.0, float(VoxelParticleNumber[v_index])/240.0);
    }
    else{
        g_out.v_pos = vec4(float(Voxel2[(v_index-300000)*voxel_memory_length+1])*h, float(Voxel2[(v_index-300000)*voxel_memory_length+2])*h, float(Voxel2[(v_index-300000)*voxel_memory_length+3])*h, 1.0);
        color = vec4(1.0, 1.0, 0.0, float(VoxelParticleNumber[v_index])/240.0);
    }
