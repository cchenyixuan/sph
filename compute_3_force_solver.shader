...
void ComputeParticleForce(){
    ...
    for(int j=0; j<96; ++j){
        ...
        if(index==index_j){continue;}
        // vector xij
        vec3 xij = Particle[index_j][0].xyz - particle_pos;
        // distance rij
        ...
        if(rij<h){
            ...
            f_pressure += grad_spiky_3d(xij.x, xij.y, xij.z, rij, h) * (Particle[index_j][1].w*(Particle[index_j][2].w/pow(Particle[index_j][2].z, 2) + Particle[index][2].w/pow(Particle[index][2].z, 2))) * Particle[index][2].z;
            // f_visco  += VISC * (        P_j_mass       /        P_j_rho        ) * (       P_j_velocity      -      P_i_velocity     ) * lap_viscosity_3d(rij, h)
            f_viscosity += VISC * (Particle[index_j][1].w / Particle[index_j][2].w) * (Particle[index_j][1].xyz - Particle[index][1].xyz) * lap_viscosity_3d(rij, h);
        }
    }
    ...
    Particle[index][3].xyz = (f_pressure + f_viscosity + f_external)*0.01/Particle[index][2].z;
}
